...
public class ConvertUtil {
    ...
    public static <T> T getValue(String value, String fieldName, Class<T> clazz) {
        ...
        } else if (clazz.getName().equals(String.class)) { // 如果获取参数值为""
            return convertGt(value, clazz);
    ...
    public static <T> T convertGt(String value, Class<T> clazz) {
        if (value == null) { // 如果值为null,则返回null
            return null;
        } else if (value.equals("") && !clazz.getName().equals(String.class)) { // 如果value值为"",而且要转为的类型不是string类型，那么就统一返回null，也就是空字符串不能转成任何其他类型的实体，只能返回null
            return null;
